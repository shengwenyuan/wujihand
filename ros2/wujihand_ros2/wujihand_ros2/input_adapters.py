...
class RosTrackerInputAdapter:
    ...
    def __init__(self, identity: TrackerInputIdentity) -> None:
        ...
        self._inbox: LatestEpochInbox[TrackedRigidBodySample] = (
            LatestEpochInbox()
        )
    ...
    def offer_message(self, message: TrackerSampleMessageLike) -> bool:
        ...
        if (
            ...
            != expected.tracking_setup_revision
    ...
    def offer_lifecycle_message(
        ...
        if (
            ...
            != self.identity.tracking_setup_revision
        ):
            self._rejected_identity += 1
            return False
        if event.kind in {
...
class RosHandObservationInputAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        self._inbox: LatestEpochInbox[CanonicalHandObservation] = (
            LatestEpochInbox()
        )
        self._started = False
        self._closed = False
    ...
    def offer_message(self, message: HandObservationMessageLike) -> bool:
        ...
        if (
            ...
            return False
        rebinds = self._inbox.metrics.rebinds
        accepted = self._offer_envelope(envelope)
        if accepted and self._inbox.metrics.rebinds > rebinds:
    ...
    def poll(
        ...
        *,
        ...
        observation = self._inbox.drain()
        if observation is None:
            raise NoHandObservationAvailable
        if (
            receive_time_ns is not None
            and observation.receive_time_ns > receive_time_ns
        ):
            self._rejected_future_time += 1
            raise NoHandObservationAvailable
        return observation
