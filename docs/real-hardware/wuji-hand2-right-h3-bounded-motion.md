# Wuji Hand2 Beta1 右手 H3 有界运动与 live 记录
...
2026-08-13 的后续 H4 台架运行在正常持续上电时达到原项目绝对线 `58 °C`，但没有伴随 fault、limit
或异常运行表现；该值也不是 Wuji 官方额定温度。经操作者确认，0.5.1 将 H3/H4 的绝对硬停止提高到
`65 °C`，同时保留相对本次启动基线 `+5 °C` 的硬停止、逐帧温度记录及其他直接安全 Gate。该调整只
适用于当前右手 Beta1、firmware `2.2.3`、SDK `2026.8.3` 的空载逐轴 bring-up，不外推为产品热规格。
